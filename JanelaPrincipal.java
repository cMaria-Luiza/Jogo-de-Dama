...
public class JanelaPrincipal extends JFrame {
    ...
    private CasaGUI novaCasaOrigem;
    private int tipoPecaOrigem;
    ...
    public void reagir(CasaGUI casaClicada) {
        ...
        if (primeiroClique) {
            ...
            if (jogo.comeu() && casaClicada != novaCasaOrigem) {
                JOptionPane.showMessageDialog(this, "Ei, babaca. Termina de comer as peças!");
                return;
            }
            
            tipoPecaOrigem = casaClicada.getTipoPeca(); 
            
            if (casaClicada.possuiPeca()) {
                // tipoPeca: se é par, é peca branca;  se é ímpar, é peca vermelha ==> diferencia a vez de cada peca
                if(jogo.suaVez(tipoPecaOrigem)) {
                    casaClicadaOrigem = casaClicada;
                    casaClicadaOrigem.destacar();
                    primeiroClique = false;
                }
                else {
                    JOptionPane.showMessageDialog(this, (jogo.getTurno() == 0 ? "Vez das peças brancas." : "Vez das peças vermelhas."));
                }
            }
            else {
                // clicou em uma posi�?o inv�lida, ent?o n?o faz nada.
                JOptionPane.showMessageDialog(this, "Clique em uma peça válida.");
            }
        }        
        else {  
            if(!(casaClicada.possuiPeca())) {
                casaClicadaDestino = casaClicada; 
                // armazena as posicoes das casas origem e destino 
                int xOrigem = casaClicadaOrigem.getPosicaoX();
                int yOrigem = casaClicadaOrigem.getPosicaoY();
                int xDestino = casaClicadaDestino.getPosicaoX();
                int yDestino = casaClicadaDestino.getPosicaoY();
                if (jogo.movimentoPermitido(xOrigem, yOrigem, xDestino, yDestino, tipoPecaOrigem)) {
                    jogo.moverPeca(xOrigem, yOrigem, xDestino, yDestino);
                    casaClicadaOrigem.atenuar();
                    // verifica se a pedra deve virar dama
                    Casa casaDestino = tabuleiro.getCasa(xDestino, yDestino);
                    if (casaDestino.getPeca().getTipo() == 0 && yDestino == 7) {
                        jogo.virarDama(casaDestino, 0);
                    }
                    else if (casaDestino.getPeca().getTipo() == 1 && yDestino == 0) {
                        jogo.virarDama(casaDestino, 1);
                    }
                    // verifica se deve continuar
                    
                    if(jogo.comeu() && jogo.deveContinuar(casaClicadaDestino)) {
                        novaCasaOrigem = casaClicadaDestino;
                    }
                    else {
                        jogo.mudarTurno();
                        jogo.naoComeu();
            ...
            // quando o usuario clicar na mesma casa, desfazer a ação
            else if (casaClicada == casaClicadaOrigem) {
                casaClicadaOrigem.atenuar();
                primeiroClique = true;
            }   
        }
